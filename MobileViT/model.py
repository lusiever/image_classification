...
class ConvLayer(nn.Module):
    ...
    def __init__(
        ...
        use_act: Optional[bool] = True,    # 是否构建激活函数
    ) -> None:
        super().__init__()
...
class InvertedResidual(nn.Module):
    ...
    def __init__(
        ...
        skip_connection: Optional[bool] = True,    # 是否使用捷径分支
    ) -> None:
        assert stride in [1, 2]
        hidden_dim = make_divisible(int(round(in_channels * expand_ratio)), 8)  # 调整特征图的通道数
        ...
        block.add_module(
            ...
            module=ConvLayer(
                ...
                stride=stride,
                kernel_size=3,
                groups=hidden_dim    # 由于使用的是depthwise conv，故卷积groups必须等于特征图的通道数
            ),
        )
        ...
        self.use_res_connect = (
            self.stride == 1 and in_channels == out_channels and skip_connection   #判断是否使用捷径分支
        )
...
class MobileViTBlock(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int,
        transformer_dim: int,   # 输入到transformer encoder block中每个token的序列长度
        ffn_dim: int,
        n_transformer_blocks: int = 2,    # 重复堆叠transformer的次数 "L"
        head_dim: int = 32,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        fm = self.conv_proj(fm)   # 1x1 conv

        fm = self.fusion(torch.cat((res, fm), dim=1))   # 3x3 conv
        return fm
